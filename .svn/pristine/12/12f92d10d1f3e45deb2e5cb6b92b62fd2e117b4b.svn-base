...
public class ExcelWriter extends BaseWriter {
	...
	private void createExcel(HttpServletResponse resp) throws IOException {
		...
		*/
		...
		String sheetName = parser.getTitle();
	...
	private void outputExcel(HttpServletResponse resp) throws IOException, WriteException {
		...
		String fileName = parser.getTitle();
		if(fileName == null || fileName.trim().length() < 1){
		    fileName = "grid";
		}
		resp.setHeader("Content-Disposition", "attachment;filename="+encodeFileName(fileName)+".xls");
	...
	private void headerPrint(ExcelXmlParser parser) throws RowsExceededException, WriteException, IOException {
        ...
        if (parser.getTitle() != null && parser.getTitle().trim().length() > 0) {
            sheet.insertRow(0);
            sheet.setRowView(0, 5000);
            WritableFont titleFont = new WritableFont(WritableFont.ARIAL, 14, WritableFont.BOLD, false);
            WritableCellFormat titleFormat = new WritableCellFormat (titleFont);
            titleFormat.setVerticalAlignment(VerticalAlignment.CENTRE);
            titleFormat.setAlignment(Alignment.CENTRE);
            Label title = new Label(0, headerOffset, parser.getTitle(), titleFormat);
            sheet.addCell(title);
            sheet.mergeCells(0, headerOffset, cols[0].length-1, headerOffset);
            headerOffset++;
        }
