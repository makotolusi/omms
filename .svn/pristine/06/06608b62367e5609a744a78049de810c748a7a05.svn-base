...
public class ProductMaintenanceModule {
	...
	public Object save(@Param("..") Product addObject) {
		...
		if (addObject.getId() == null) {
			addObject.setProductCode("");
			Result re=  productService.insert(addObject, "操作成功", "操作失败");
			TypeMaintain type=typeMaintainService.fetch(Integer.parseInt(addObject.getPieceCategory()));
			Long id=Long.parseLong(re.getAttr().get("id").toString());
			addObject.setProductCode(type.getType()+id);
			addObject.setId(id);
			productService.update(addObject, "操作成功", "操作失败");
			return re;
		}
		Product queryObj = productService.fetch(addObject.getId());
		String productCode = addObject.getProductCode();
