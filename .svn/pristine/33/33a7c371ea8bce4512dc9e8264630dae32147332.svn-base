...
public class AgentModule {
	...
	public Result relation(@Param("..") CommerceUserAgentRelation agentRelation) {
		CommerceUserAgentRelation agent = agentRelationService.fetch(Cnd.where("offlineId",
				"=", agentRelation.getOffLineId()));//("agentId", "=", agentRelation.getAgentId()).and
		if (agent == null) {
			agentRelation.setStatus(Status.NORMAL);
			agentRelationService.insert(agentRelation);
			return Result.ok("成功");
		}else
			return Result.ok("该会员已有所属经销商!");
