...
public class ActivityExtModule {
	...
	public Object save(@Param("..") Activity addObject) {
		...
		if (addObject.getId() == null) {
			addObject.setRushStatus(Activity.Status.STOP.getValue() + "");
	...
	public Result delete(Long id) {
		Result result=activityService.delete(id, "删除成功", "删除失败");
		List<ActivityProductData> ac=activityProductDataService.query(Cnd.where("activityId", "=", id), new Pager());
		for (Iterator iterator = ac.iterator(); iterator.hasNext();) {
			ActivityProductData activityProductData = (ActivityProductData) iterator
					.next();
			if(activityProductDataService.delete(activityProductData.getId())){
				activityProductDataService.delActivityProductFromCache(activityProductData);//清除缓存
			}
		}
		return result;
	}
	...
	/** 删除记录 */
	@At({ "/activityproductdelete", "/activityproductdelete/?" })
	@DELETE
	public Result activityproductdelete(Long id) {
		ActivityProductData ac=activityProductDataService.fetch(id);
		Result result = activityProductDataService.delete(id, "删除成功", "删除失败");
		if (result.isSuccess()) {// 关闭活动清除缓存
				activityProductDataService.delActivityProductFromCache(ac);
		}
		return result;
	}
	...
	public DataGrid items(String lk_activityName) {
		...
		for (Activity act : (LinkedList<Activity>) dg.getRows()) {
			String url = picHandlerService.getQiniuPrivateUrl(act.getImgUrl());
	...
	public Object updActivityStatus(@Param("..") Activity object) {
		...
		Result result = activityService.update(obj, "操作成功", "操作失败");
		if (result.isSuccess()) {// 关闭活动清除缓存
			if ("1".equals(obj.getRushStatus())) {
				activityProductDataService.addActivityToCache(obj);
			} else {
				activityProductDataService.delActivityCache(obj);
			}
		}
		return result;
	}
	...
	public Object updActivityProduct(@Param("..") ActivityProductData object) {
		...
		ActivityProductData obj = (ActivityProductData) activityProductDataService
				.fetch(object.getId());
		...
		Result result = activityProductDataService.update(obj, "操作成功", "操作失败");
		if (result.isSuccess()) {
			activityProductDataService.addActivityProductToCache(obj);
		}
		return result;
	}
	...
	@POST
	public Object updActivityProductStatus(
			@Param("..") ActivityProductData object) {
		// Cnd cnd = Cnd.limit();
		// cnd.and("id", "=", object.getId());
		ActivityProductData obj = (ActivityProductData) activityProductDataService
				.fetch(object.getId());
		...
		if (result.isSuccess()) {// 关闭活动清除缓存
			if ("1".equals(object.getStatus()))
				...
				activityProductDataService.delActivityProductFromCache(obj);
		}
		return result;
	}
	...
	public Result closeActivityProductList(Long activityId) {
		Cnd cnd = Cnd.limit().and("activityId", "=", activityId);
		List<ActivityProductData> activityProductDataList = activityProductDataService
				.query(cnd, null);
	...
	@POST
	public Map yesguanlianproduct(Long activityId, String productIds,
			String productCodes) {
		...
		for (int i = 0; i < productIdStrArr.length; i++) {
			...
			// get product name
			Product tempProduct = productService.fetch(Long
					.parseLong(productIdStrArr[i]));
			if (tempProduct != null) {
				String productName = tempProduct.getName();
				addObject.setProductName(productName);
			} else {
				result.put("failure", true);
				result.put("message", "没有找到这个产品的名字， product ID ="
						+ productIdStrArr[i]);
				return result;
			}
			...
			retVal = activityProductDataService.insert(addObject, "操作成功",
					"操作失败");
			if (retVal.isSuccess() == false) {
				result.put("success", false);
				return result;
			} else {// 插入成功需要将数据保存进缓存 抢购时使用
				activityProductDataService.addActivityProductToCache(addObject);
			}
		}
