...
public class FileModule extends BasicModule {
	...
	@POST
	@AdaptBy(type = UploadAdaptor.class, args = { "${app.root}/WEB-INF/tmp" })
	public Object upload(@Param("imgFile") TempFile imgFile,
		...
		String key = "test/"
				+ String.valueOf(random.nextLong()).replace("-", "")
						.substring(1, 5) + imgFile.getFile().getName();
		try {
	...
	@POST
	@AdaptBy(type = UploadAdaptor.class, args = { "${app.root}/WEB-INF/tmp" })
	public KeResult upload(AttaType type, @Param("imgFile") TempFile file,
			@Param("fileDesc") String fileDesc) {

		HashMap<String, Object> result = (HashMap<String, Object>) this.upload(
				file, null, null);

		String tokenUrl = picHandlerService.getQiniuPrivateUrl(result
				.get("url").toString());
		PieceMaintain pm = new PieceMaintain();
		pm.setImgUrl(result.get("url").toString());
		KeResult kr = KeResult.ok(tokenUrl, pm);
